...
import com.itheima.mobliesafe.WatchDogActivity;
...
public class WatchDogService extends Service {
	...
	public void onCreate() {
		...
		intentFilter.addAction("com.itheima.mobliesafe.unlock");
		// 3.注册广播接受者
		...
		timer.schedule(new TimerTask() {
			...
			public void run() {
				...
				for (RunningTaskInfo runningTaskInfo : runningTasks) {
					...
					/*
					 * //获取任务栈栈顶的activity runningTaskInfo.topActivity;
					 */
					String packageName = baseactivity.getPackageName();
					System.out.println("WatchDogServive,当前栈顶:"+packageName);
					// 通过查询数据库,如果数据库中有,弹出解锁界面,没有不作处理
					if (list.contains(packageName)) {// 判断list集合中是否包含包名
						if (!unlockcurrentPackagname.contains(packageName)) {
							// 弹出解锁界面
