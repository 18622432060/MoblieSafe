...
import com.itheima.mobliesafe.db.dao.AntiVirusDao;
import com.itheima.mobliesafe.utils.MD5Util;

public class AntivirusActivity extends Activity {

	@InjectView(R.id.iv_antivirus_scanner)
	ImageView iv_antivirus_scanner;
	
	@InjectView(R.id.pb_antivirus_progressbar)
	ProgressBar pb_antivirus_progressbar;
	
	@InjectView(R.id.tv_antivirus_text)
    TextView tv_antivirus_text;
	
	@InjectView(R.id.ll_antivirus_safeapks)
	LinearLayout ll_antivirus_safeapks;
				...
				for (final PackageInfo packageInfo : installedPackages) {
					...
					runOnUiThread(new Runnable() {//封装了handler
						...
						public void run() {
							...
							//textview添加到线性布局中
							//ll_antivirus_safeapks.addView(textView);//添加到底部
							ll_antivirus_safeapks.addView(textView, 0);//添加到上面 index:将textview添加到线性布局的哪个位置
						}
					});
				};
				...
				runOnUiThread(new Runnable() {
					...
					public void run() {
						...
						if (list.size() > 0) {
							...
							builder.setIcon(R.drawable.dead_by_daylight);
